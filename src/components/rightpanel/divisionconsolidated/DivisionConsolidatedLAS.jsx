...
const DivisionConsolidatedLAS = ({ selectedDivision }) => {
    ...
    const loadDivisionData = async () => {
      ...
      try {
        ...
        if (!validFiles.length) {
          setErrorText(
            "schools.json was found, but no las.json files could be loaded."
          );
          setRows([]);
          setLoading(false);
          return;
        ...
        console.error("Error loading consolidated LAS data:", error);
        setErrorText(error.message || "Failed to load division LAS data.");
        setRows([]);
      } finally {
        setLoading(false);
      }
  ...
  const divisionSummary = useMemo(() => {
    return accordionSchools.reduce(
      (summary, school) => ({
        ...
        enrolled: summary.enrolled + toNumber(school.totals.enrolled),
        received: summary.received + toNumber(school.totals.received),
        gaps: summary.gaps + toNumber(school.totals.gap),
        surplus: summary.surplus + toNumber(school.totals.surplus),
      }),
      {
    ...
    <section className="dctWrap">
      <div className="dctHeader">
        <div className="dctHeaderLeft">
          <div className="dctTitle">DIVISION CONSOLIDATED LAS DATA</div>
      ...
      <div className="dctAccordionWrap">
        {loading ? (
          <div className="dctEmpty">Loading consolidated LAS data...</div>
        ) : errorText ? (
          <div className="dctEmpty">{errorText}</div>
        ) : accordionSchools.length === 0 ? (
          <div className="dctEmpty">No LAS data found for this division.</div>
        ) : !showPerSchool ? (
                ...
                <div className="dctAccordionHeaderShell">
                  ...
                  <div className="dctAccordionMeta">
                    ...
                    <div className="dctAccordionHeaderRight">
                      ...
                      <span className="dctAccordionStat">
                        <span className="dctAccordionStatLabel">LAS Received</span>
                        <span className="dctAccordionStatValue">
...
export default DivisionConsolidatedLAS;
